...
def setup():
    try:
        ...
        grf_file = c3d_path.replace('.c3d', '\\forces\\' + file_name_no_ext + '\\grf.mot')
...
def run_so(osim_model_path,trial_path):
    try:
        trial = msk.bops.Trial(trial_path)
        msk.bops.run_SO(osim_model_path, trial.markers, trial.ik, trial.id, trial.so_force)
    except Exception as e:
...
if __name__ == '__main__':
    ...
    osim_model_path = r"C:\Git\research_documents\students\marcel_BSc_vienna\opensim-deadlift-techniques\athlete_0\model\jointcenters\scale_model.osim"
    trial = msk.bops.Trial(trial_path)
